...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="input-group input-group-static mb-4">
                <label>Sale</label>
                <input type="sale" value="{{ old('sale') ?? $product->sale }}" name="sale" class="form-control">
                @error('sale')
                    <span class="text-danger"> {{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="input-group input-group-static mb-4">
                ...
                <textarea type="text" style="width: 100%" name="description" id="description" class="form-control">{{ old('description') ?? $product->description }}   </textarea>
                @error('description')
                    <span class="text-danger"> {{ $message }}</span>
                @enderror
            </div>
