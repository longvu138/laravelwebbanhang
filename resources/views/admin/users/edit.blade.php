...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-5">
                    <img src="{{ $user->images->count() > 0 ? asset('upload/' . $user->images->first()->url) : 'upload/default.png' }}"
                        id="show-image" alt="" style="width: 150px;">
                </div>
            </div>
            ...
            <div class="input-group input-group-static mb-4">
                <label>Address</label>
                <textarea type="text" name="address" class="form-control">{{ old('address') ?? $user->address }} </textarea>
                @error('address')
                    <span class="text-danger"> {{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="input-group input-group-static mb-4">
                ...
                <select name="gender" class="form-control" value={{ $user->gender }}>
                    ...
                    <option {{ $user->gender === 'fe-male' ? 'selected' : '' }} value="fe-male">FeMale</option>
                </select>
